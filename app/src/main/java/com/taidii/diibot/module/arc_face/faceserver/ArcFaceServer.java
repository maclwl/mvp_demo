...
public class ArcFaceServer {
    ...
    public static ArcFaceServer getInstance() {
        if (arcFaceServer == null) {
            synchronized (ArcFaceServer.class) {
                if (arcFaceServer == null) {
                    arcFaceServer = new ArcFaceServer();
                }
            }
        }
        return arcFaceServer;
    }
     ...
     *
     * @param context 上下文对象
     * @return 是否初始化成功(本地存储人脸信息库方式两种 一.文件存储 二.数据库存储 当前采用数据库 )
     */
    ...
    /**
     * 初始化人脸特征数据以及人脸特征数据对应的注册图(文件存储人脸信息库)
     *
     * @param context 上下文对象
     */
    private void initFileFaceList(Context context) {
        synchronized (this) {
            if (ROOT_PATH == null) {
    ...
    /**
     * 初始化本地人脸特征数据库（本地数据库存储人脸信息库）
     */
    private void initSqlFaceList(){
        synchronized (this) {
            faceRegisterInfoList = new ArrayList<>();
            faceRegisterInfoList = OrmDBHelper.getInstance().getQueryAll(FaceRegisterInfo.class);
        }
    }
     ...
     * 直接保存特征数据
     * @param context 上下文对象
     * @param faceFeature   特征数据
     * @param name    保存的名字，若为空则使用时间戳
     * @return 是否注册成功
     */
    public boolean registerFaceFeature(Context context, FaceFeature faceFeature, String name,String faceImage){
        synchronized (this) {
            if (faceEngine == null || context == null || faceFeature == null ) {
                Log.e(TAG, "registerFaceFeature:  invalid params");
                return false;
            }
            ...
            File featureDir = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR);
            if (!featureDir.exists() && !featureDir.mkdirs()) {
                Log.e(TAG, "registerFaceFeature: can not create feature directory");
                return false;
            }
            try {
                // 保存特征数据
                ...
                faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), userName,faceImage));
                return true;
            } catch (IOException e) {
                e.printStackTrace();
                return false;
     ...
     * @param width   bgr24宽度
     * @param height  bgr24高度
     * @param name    保存人脸对应的名字
     * @param id      保存人脸对应的id
     * @return 是否注册成功
     */
    public boolean registerFaceBgr24(Context context, byte[] bgr24, int width, int height, String name,int id,String faceImage,String faceType){
        synchronized (this) {
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                String userName = name == null ? String.valueOf(System.currentTimeMillis()) : name;
                if (code == ErrorInfo.MOK) {
                    FaceRegisterInfo faceRegisterInfo = new FaceRegisterInfo(faceFeature.getFeatureData(), userName,faceImage);
                    faceRegisterInfo.setId(id);
                    faceRegisterInfo.setType(faceType);
                    OrmDBHelper.getInstance().save(faceRegisterInfo);
     ...
     * @return 是否注册成功
     */
    public boolean registerFaceNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name, int id, String faceImage){
        synchronized (this) {
            if (faceEngine == null || context == null || nv21 == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                Log.e(TAG, "registerNv21: invalid params");
                return false;
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        if (maxSimilarIndex != -1) {
            CompareResult compareResult = new CompareResult(faceRegisterInfoList.get(maxSimilarIndex).getName(), maxSimilar);
            compareResult.setId(faceRegisterInfoList.get(maxSimilarIndex).getId());
            compareResult.setType(faceRegisterInfoList.get(maxSimilarIndex).getType());
    ...
    public Bitmap getNv21Bitmap(Context context, byte[] nv21, int width, int height,FaceInfo faceInfo){
        Rect cropRect = getBestRect(width, height, faceInfo.getRect());
        if (cropRect == null) {
            Log.e(TAG, "registerNv21: cropRect is null!");
        }
