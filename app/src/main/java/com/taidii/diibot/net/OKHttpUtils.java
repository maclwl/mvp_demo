...
public final class OKHttpUtils {
            ...
            public void run() {
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        Object t = gson.fromJson(jsonString,onResponse.getType());
                        // 标记请求里的额外成功
                        onResponse.setSuccess(true);
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
                                onResponse.onSuccess((T) t);
                                onResponse.onCompleted();
                            }
                        });
                    } else {
    ...
    public static <T> void postFormForQiNiu(String url, ArrayMap<String, String> params, Object tag, OnResponse<T>
            onResponse) {
    ...
    public static <T> void postFormNoToken(String url, ArrayMap<String, String> params, Object tag, OnResponse<T> onResponse) {
        // 构造Multipart请求体，并设置类型为Form
        MultipartBody.Builder multiBuilder = new MultipartBody.Builder().setType(MultipartBody
                .FORM);
        // params是存放参数的ArrayMap
    ...
    public static <T> void get(String url, ArrayMap<String, String> headers, ArrayMap<String,
            String> params, Object tag, final OnResponse<T> onResponse) {
        if (headers == null) {
            headers = new ArrayMap<>();
        }
        headers.put("Authorization", TOKEN_PREFIX + GlobalParams.token);
    ...
    public static <T> void get(String url, ArrayMap<String, String> params, Object tag,
    ...
    public static <T> void put(String url, JsonObject params, Object tag, OnResponse<T>
            onResponse) {
        final RequestBody reqBody = RequestBody.create(MEDIA_TYPE_JSON, params == null ?
                EMPTY_JSON : params.toString());
        url = checkUrlPrefix(url);
    ...
    public static <T> void put(String url, ArrayMap<String, String> params, Object tag,
                               OnResponse<T> response) {
        post(url, params, tag, METHOD_PUT, response);
    }

    public static void download(final String fileUrl, final String targetPath, final String
            targetFileName, final DownloadHandler downloadHandler) {
    ...
    public static <T> void deleteNoToken(String url, Object tag, OnResponse<T> onResponse) {
        url = checkUrlPrefix(url);
        Request.Builder builder = new Request.Builder().url(url);
        if (tag != null) {
            if (tag instanceof JsonObject) {
    ...
    public static <T> void patch(String url, JsonObject params, Object tag, OnResponse<T>
            onResponse) {
        final RequestBody reqBody = RequestBody.create(MEDIA_TYPE_JSON, params == null ?
                EMPTY_JSON : params.toString());
        url = checkUrlPrefix(url);
    ...
    public static void downloadSync(String downloadUrl, Object tag, final String downloadPath,
                                    final String fileName, final DownloaderListener listener) {
    ...
    public static void download(String downloadUrl, Object tag, final String downloadPath, final
    String fileName, final DownloaderListener listener) {
        OkHttpClient tmpClient = downloadClient.newBuilder()
                .addInterceptor(new Interceptor() {
                    @Override
                    public Response intercept(Chain chain) throws IOException {
    ...
    public abstract static class OnResponse<T> {
        ...
        public void onFailed(int code, String msg, String url) {
            switch (code) {
                ...
                default:
                    // PromptManager.showToast(R.string.request_error);
                    Toast.makeText(BaseApplication.getInstance(), R.string.network_timeout, Toast.LENGTH_SHORT).show();
                    break;
            }
        }
        ...
        Type getType(){
            ...
            if(type instanceof Class){
                return type;//如果是{}直接返回
            }else{
                return new TypeToken<T>(){}.getType();//如果是[]，获取集合的类型map或list
            }
        }
    }
